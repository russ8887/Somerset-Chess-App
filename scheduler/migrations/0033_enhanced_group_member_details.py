...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('scheduler', '0032_unlimited_results_with_optimal_scoring'),
    ]

    operations = [
        migrations.RunSQL(
            sql="""
            -- Drop the previous function and create the enhanced version with group member details
            ...
            BEGIN
                ...
                -- PART 1: DIRECT PLACEMENTS (ALL compatible groups with space, with member details)
                RETURN QUERY
                WITH 
                ...
                SELECT 
                    ...
                    ('Direct placement - ' || cg.current_size || '/' || cg.max_capacity || ' students, ' ||
                     ...
                     CASE 
                        WHEN cg.current_size = 1 THEN 'IDEAL: Join ' || (cg.current_members->0->>'student_name') || ' (' || (cg.current_members->0->>'skill_level') || ', ' || (cg.current_members->0->>'enrollment_type') || ')'
                ...
                IF include_displacements AND result_count < max_results THEN
                    ...
                    -- VERY LOW threshold for displacement (was +5, now +2)
                    AND fg.target_compatibility_score > COALESCE((fg.displacement_candidate->>'compatibility_score')::INTEGER, 0) + 2
