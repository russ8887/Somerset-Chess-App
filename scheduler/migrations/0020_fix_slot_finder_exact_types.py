...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('scheduler', '0019_fix_slot_finder_return_types'),
    ]

    operations = [
        migrations.RunSQL(
            sql="""
            -- Drop and recreate the advanced slot finder function with exact database types
            ...
            BEGIN
                ...
                IF student_enrollment_type IS NULL THEN
                ...
                compatibility_scores AS (
                    SELECT 
                        ga.*,
                        (ts.start_time || ' - ' || ts.end_time)::TEXT as time_display,
                        CASE ga.day_of_week
                            WHEN 0 THEN 'Monday'
                            WHEN 1 THEN 'Tuesday' 
                ...
                displacement_opportunities AS (
                    SELECT 
                        cs.*,
                        'displacement'::TEXT as placement_type,
                        json_build_object(
                            'type', 'displacement',
                        ...
                        ('Displacement - move ' || cs.current_size || ' student(s) to optimize placement')::TEXT as explanation
                    FROM compatibility_scores cs
