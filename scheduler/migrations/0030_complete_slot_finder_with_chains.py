...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('scheduler', '0029_clean_rebuild_slot_finder'),
    ]

    operations = [
        migrations.RunSQL(
            sql="""
            -- Drop the diagnostic function and create the complete slot finder
            ...
            BEGIN
                ...
                -- PART 1: DIRECT PLACEMENTS (groups with available space)
                RETURN QUERY
                WITH 
                ...
                available_groups AS (
                    SELECT 
                        ...
                        sg.max_capacity, sg.target_skill_level,
                        ...
                        (
                            ...
                            END +
                            CASE 
                                WHEN COUNT(sgm.enrollment_id) = 0 THEN 50  -- Empty group bonus
                                WHEN COUNT(sgm.enrollment_id) < sg.max_capacity * 0.7 THEN 30  -- Underutilized
                                ELSE 20  -- Normal capacity
                            END +
                            CASE 
                                WHEN sg.id != COALESCE(student_current_group_id, -1) THEN 20  -- Different group bonus
                                ELSE 0
                            END
                    ...
                    GROUP BY sg.id, sg.name, sg.day_of_week, sg.time_slot_id, sg.max_capacity, 
                             sg.target_skill_level, u.first_name, u.last_name, ts.start_time, ts.end_time
                    HAVING COUNT(sgm.enrollment_id) < sg.max_capacity  -- Only groups with space
                )
                SELECT 
                    ag.time_slot_id::BIGINT as slot_id, ag.group_id::BIGINT,
                    ag.group_name::VARCHAR(100), ag.coach_name::VARCHAR(202),
                    ag.day_name::VARCHAR(10), ag.time_display::VARCHAR(50) as time_slot,
                    ag.compatibility_score, 'direct'::VARCHAR(20) as placement_type,
                    ag.current_size, ag.max_capacity,
                    json_build_object(
                        'type', 'direct',
                        'current_members', ag.current_size,
                        'available_space', ag.max_capacity - ag.current_size,
                        'skill_match', CASE 
                            WHEN student_skill_level = ag.target_skill_level THEN 'perfect'
                            WHEN ABS(ASCII(student_skill_level) - ASCII(ag.target_skill_level)) = 1 THEN 'good'
                            ELSE 'acceptable'
                        END
                    ) as displacement_info,
                    ('Direct placement - ' || ag.current_size || '/' || ag.max_capacity || ' students, ' ||
                     CASE 
                        WHEN student_skill_level = ag.target_skill_level THEN 'perfect skill match'
                        ELSE 'skill level ' || ag.target_skill_level || ' group'
                ...
                -- PART 2: DISPLACEMENT PLACEMENTS (if requested and we have room for more results)
                IF include_displacements AND result_count < max_results THEN
                    ...
                    full_groups_with_displacement_candidates AS (
                        SELECT 
                            sg.id as group_id, sg.name as group_name, sg.day_of_week, sg.time_slot_id,
                            sg.max_capacity, sg.target_skill_level,
                            (u.first_name || ' ' || u.last_name)::VARCHAR(202) as coach_name,
                            ...
                            CASE sg.day_of_week
                                ...
                                ELSE 'Unknown'::VARCHAR(10)
                            END as day_name,
                            -- Find the least compatible student in this group for potential displacement
                            (
                    ...
                    SELECT 
                        ...
                        (fg.target_compatibility_score - 
                         COALESCE((fg.displacement_candidate->>'compatibility_score')::INTEGER, 0) / 2)::INTEGER as feasibility_score
                    FROM full_groups_with_displacement_candidates fg
                    WHERE fg.displacement_candidate IS NOT NULL
                    -- Only suggest displacement if target student is significantly more compatible
                    AND fg.target_compatibility_score > COALESCE((fg.displacement_candidate->>'compatibility_score')::INTEGER, 0) + 20
