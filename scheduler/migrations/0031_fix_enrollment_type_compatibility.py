...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('scheduler', '0030_complete_slot_finder_with_chains'),
    ]

    operations = [
        migrations.RunSQL(
            sql="""
            -- Drop the previous function and create the complete, business-rule-compliant slot finder
            ...
            BEGIN
                ...
                -- PART 1: DIRECT PLACEMENTS (groups with available space and compatible enrollment types)
                RETURN QUERY
                WITH 
                ...
                compatible_groups AS (
                    SELECT 
                        ...
                        -- Enhanced compatibility scoring with enrollment type consideration
                        (
                            CASE 
                            ...
                            END +
                            CASE 
                                WHEN student_enrollment_type = 'INDIVIDUAL' AND COUNT(sgm.enrollment_id) = 0 THEN 100  -- Perfect for individual
                                WHEN student_enrollment_type != 'INDIVIDUAL' AND COUNT(sgm.enrollment_id) > 0 THEN 30  -- Good for pair/solo
                                ELSE 0
                            END
                ...
                SELECT 
                    ...
                    ('Direct placement - ' || cg.current_size || '/' || cg.max_capacity || ' students, ' ||
                     ...
                     END || 
                     CASE 
                        WHEN cg.enrollment_compatible THEN ', compatible enrollment types'
                        ELSE ', INCOMPATIBLE enrollment types'
                ...
                ORDER BY cg.compatibility_score DESC, cg.coach_name  -- Add coach diversity
                ...
                IF include_displacements AND result_count < max_results THEN
                    ...
                    -- Lower threshold for displacement (was +20, now +10)
                    AND fg.target_compatibility_score > COALESCE((fg.displacement_candidate->>'compatibility_score')::INTEGER, 0) + 10
